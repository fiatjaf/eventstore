...
func (it *iterator) seek(key []byte) {
	if _, _, errsr := it.cursor.Get(key, nil, mdbx.SetRange); errsr != nil {
		if !mdbx.IsNotFound(errsr) {
			...
			it.key, it.valIdx, it.err = it.cursor.Get(nil, nil, mdbx.Last)
		}
	} else {
		// move one back as the first step
		it.key, it.valIdx, it.err = it.cursor.Get(nil, nil, mdbx.Prev)
	}
}
...
func (b *MDBXBackend) keyName(key key) string {
	return fmt.Sprintf("<dbi=%s key=%x>", b.dbiName(key.dbi), key.key)
}

func (b *MDBXBackend) getIndexKeysForEvent(evt *nostr.Event) iter.Seq[key] {
...
func (b *MDBXBackend) getTagIndexPrefix(tagValue string) (mdbx.DBI, []byte, int) {
...
func (b *MDBXBackend) dbiName(dbi mdbx.DBI) string {
	switch dbi {
	case b.hllCache:
		return "hllCache"
