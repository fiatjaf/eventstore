...
func (b *MDBXBackend) QueryEvents(ctx context.Context, filter nostr.Filter) (chan *nostr.Event, error) {
	...
	go b.mdbxEnv.View(func(txn *mdbx.Txn) error {
		defer close(ch)
...
func (b *MDBXBackend) query(txn *mdbx.Txn, filter nostr.Filter, limit int) ([]internal.IterEvent, error) {
	...
	for c := 0; ; c++ {
		...
		for q, query := range queries {
			...
			for {
				...
				if err != nil {
					log.Printf(
						"mdbx: failed to get %x based on prefix %x, index key %x from raw event store: %s\n",
				...
				if err := bin.Unmarshal(val, event); err != nil {
					log.Printf("mdbx: value read error (id %x) on query prefix %x sp %x dbi %d: %s\n", val[0:32],
