...
func (b *EdgeDBBackend) SaveEvent(ctx context.Context, event *nostr.Event) error {
	...
	query := "INSERT Event { eventId := <str>$eventId, pubkey := <str>$pubkey, createdAt := <datetime>$createdAt, kind := <int64>$kind, tags := array<json>$tags, content := <str>$content, sig := <str>$sig }"
